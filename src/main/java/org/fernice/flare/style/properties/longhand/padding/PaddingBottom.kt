...
object PaddingBottomId : AbstractLonghandId<PaddingBottomDeclaration>(
    name = "padding-bottom",
    declarationType = PaddingBottomDeclaration::class,
    isInherited = false,
) {

    override fun parseValue(context: ParserContext, input: Parser): Result<PaddingBottomDeclaration, ParseError> {
        return NonNegativeLengthOrPercentage.parse(context, input).map { PaddingBottomDeclaration(it) }
    }

    override fun cascadeProperty(context: Context, declaration: PaddingBottomDeclaration) {
...
class PaddingBottomDeclaration(val length: NonNegativeLengthOrPercentage) : PropertyDeclaration(
    id = PropertyDeclarationId.Longhand(PaddingBottomId),
) {
