...
class BorderTopLeftRadiusId : LonghandId() {
    ...
    override fun parseValue(context: ParserContext, input: Parser): Result<PropertyDeclaration, ParseError> {
        return LengthOrPercentage.parse(context, input).map { width -> BorderTopLeftRadiusDeclaration(width) }
    }
...
class BorderTopLeftRadiusDeclaration(val radius: LengthOrPercentage) : PropertyDeclaration() {
