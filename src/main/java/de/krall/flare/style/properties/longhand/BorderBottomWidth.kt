...
class BorderBottomWidthId : LonghandId() {
    ...
    override fun parseValue(context: ParserContext, input: Parser): Result<PropertyDeclaration, ParseError> {
        return NonNegativeLength.parse(context, input).map { width -> BorderBottomWidthDeclaration(width) }
    }
...
class BorderBottomWidthDeclaration(val width: NonNegativeLength) : PropertyDeclaration() {
